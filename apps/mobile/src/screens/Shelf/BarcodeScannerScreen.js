...
// ─── SERVICES & MODELS (Adjust paths to your project structure) ──────────
...
export default function BarcodeScannerScreen({ navigation }) {
    ...
    const handleSaveBook = async () => {
        ...
        try {
            // 1. Generate the user-specific book model
            const myNewBook = createUserBookModel(currentUser.uid, scannedBook, 'reading'); // Or 'want_to_read'
            
            // 2. Save it directly to Firestore!
    ...
    const resetScanner = () => {
        ...
        setScannedBook(null);
        // Small delay to allow modal to close before re-activating camera
    ...
    return (
        <View style={[localStyles.root, { backgroundColor: theme.background }]}>
            ...
            <View style={localStyles.cameraContainer}>
                <CameraView
                    ...
                    barcodeScannerSettings={{
                        barcodeTypes: ["ean13", "ean8", "upc_e", "upc_a"], // Standard book formats
                    }}
            ...
            {/* ── Result Modal (Using your EditProfile style) ── */}
            <Modal animationType="slide" transparent={true} visible={modalVisible} onRequestClose={resetScanner}>
                <View style={localStyles.modalOverlay}>
                    <View style={[localStyles.modalContent, { backgroundColor: theme.background || '#FFF' }]}>
                        ...
                        {scannedBook ? (
                            ...
                            <>
                                ...
                                <Text style={[localStyles.errorText, { color: theme.subtext }]}>
                                    We couldn't find this ISBN in the Google Books database.
                                </Text>
                                
                                <TouchableOpacity style={localStyles.primaryButton} onPress={() => {
                                    setModalVisible(false);
                                    navigation.navigate('ManualIsbnScreen'); // Or wherever they can enter manually
                                }}>
                        ...
                        {/* Always show a cancel/rescan button */}
                        <TouchableOpacity 
...
const localStyles = StyleSheet.create({
    ...
    header: {
        ...
        paddingTop: 60, // Adjust for notch/status bar if needed
        paddingBottom: 20,
        zIndex: 10,
    },
    headerTitle: {
    ...
    // Corner markers for the scanner target
    corner: {
        position: 'absolute',
        width: 30,
        height: 30,
    ...
    // Modal Styles (Matches EditProfileScreen)
    modalOverlay: {
        flex: 1,
