// ISO 3166-1 alpha-2 → dial code + name
// Flag emoji is derived at runtime from the ISO code (see countryFlag util)
...
export const DEFAULT_COUNTRY = COUNTRIES.find(c => c.code === 'PT');

// Derives a flag emoji from a 2-letter ISO 3166-1 alpha-2 code.
export const countryFlag = (code) =>
    [...code.toUpperCase()].map(c =>
        String.fromCodePoint(0x1F1E0 + c.charCodeAt(0) - 65)
    ).join('');

// Returns the best-matching country for a stored phone string (e.g. "+351 912345678").
export const parseStoredPhone = (stored) => {
    if (!stored) return { country: DEFAULT_COUNTRY, number: '' };
    const trimmed = stored.trim();
    if (!trimmed.startsWith('+')) return { country: DEFAULT_COUNTRY, number: trimmed };
    // Try longest dial match first to avoid e.g. +1 matching +1868
